...
import argparse, json, time, threading, random, os
import requests
...
def make_prefix(seed, nwords):
    ...
    return "Document " + str(seed) + ": " + " ".join(
        r.choice(VOCAB) + str(r.randint(0, 999)) for _ in range(nwords))

def ttft_once(prompt):
    t0 = time.perf_counter()
    try:
        r = requests.post(URL, json={"model": MODEL, "prompt": prompt, "max_tokens": 4,
                                     "temperature": 0, "stream": True}, stream=True, timeout=300)
    except Exception:
        return None
    ttft = None
...
def main():
    ...
    def worker():
        while time.time() < stop:
            with lock:
                p = prefixes[idx[0] % a.nprefix]; idx[0] += 1
            t = ttft_once(p)
            if t is not None:
                with lock:
                    ttfts.append(t)
    ...
    for th in threads: th.join(timeout=a.duration + 40)
    ...
    if n == 0:
        ...
        return
    p50 = ttfts[n // 2]; p99 = ttfts[min(n - 1, int(0.99 * n))]
    print(f"RESULT arm={a.arm} C={a.concurrency} n={n} "
          f"TTFT_p50={p50*1000:.0f}ms TTFT_p99={p99*1000:.0f}ms "
          f"tput={n/elapsed:.2f}req/s", flush=True)

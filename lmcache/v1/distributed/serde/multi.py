...
The base :class:`Serializer` / :class:`Deserializer` interfaces in
``serde/base.py`` model a single source MemoryObj going to or coming
from a single byte-buffer MemoryObj. That single-tensor shape covers
symmetric per-tensor quantization (e.g., the built-in ``fp8`` serde),
where the engine hands the L2 path one KV-shaped tensor and gets one
KV-shaped tensor back.

Some quantization schemes need a tuple of tensors at one of the
endpoints. Asymmetric K16/V8 is the motivating example: the
deserialized form is a triple ``(K_native, V_fp8, V_scales)`` so that
an asymmetric paged KV cache can consume V without re-expanding it
back to the native dtype. V-only writes are the dual: when K is
already resident in HBM, only V (and its scales) need to ship to L2,
which the serializer expresses as a tuple with the K slot left
absent.

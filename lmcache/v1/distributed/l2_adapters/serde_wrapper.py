...
class _KChildSlab(MemoryAllocatorInterface):
    ...
    * ``free``: rebind the ``MemoryObj`` for reuse (reset
    ...
    def __init__(self, shape: torch.Size, dtype: torch.dtype, max_slots: int) -> None:
        ...
        # policy can't see real K-child pressure (measurement
        # confirmed this).
        self._in_flight = 0
        self._alloc_lock = threading.Lock()
    ...
    def get_used_capacity_bytes(self) -> tuple[int, int]:
        ...
        K-child memory pressure, not just the pooled subset.  The prior
        formula (allocated_count - free_count) silently dropped the
        one-shots, leaving the eviction policy blind to most of the
        K-child footprint under heavy producer load.
    ...
    def _finalize_split_tier_success(self, state: _StoreTaskState) -> None:
        ...
        for logical_key in state.keys:
            try:
                ...
                # controller.  The K child + manifest entry

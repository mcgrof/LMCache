...
class _VScratchSlab:
    ...
    def __init__(self, shape: torch.Size, dtype: torch.dtype, max_slots: int) -> None:
        ...
        # rather than just the pooled subset (measurement
...
class _KChildSlab(MemoryAllocatorInterface):
    ...
    * ``free``: rebind the ``MemoryObj`` for reuse (reset
    ...
    def get_used_capacity_bytes(self) -> tuple[int, int]:
        ...
        K-child memory pressure, not just the pooled subset.  The
...
class SerdeL2AdapterWrapper(L2AdapterInterface, EarlyReleaseStoreAdapter):
    ...
    def submit_lookup_and_lock_task(
        ...
        if self._placement_mode == StoragePlacementMode.KV_SPLIT_TIER:
            ...
            return self._inner.submit_lookup_and_lock_task(v_child_keys, layout_desc)
    ...
    def _alloc_split_tier_children(
        ...
        * Commit the K child write so it persists as the canonical
        ...
        # (pod regression discovered 2026-05-31).
        #

...
class ScaleScope(IntEnum):
    ...
    `per_layer_head` one FP32 scalar per head, applied to all pages
                     of that layer.  This is what the asymmetric vLLM
                     branch's `_v_scale_float` exposes today.
...
def _unpack_str(buf: memoryview, off: int) -> Tuple[str, int]:
    if off + 2 > len(buf):
        raise CorruptEncodedKVError(
            f"truncated string-length field at offset {off}"
        )
...
class EncodedKV:
    ...
    # at serialize time.  When parsed from a buffer, both are filled.
    payload: bytes = b""
...
def _int_to_dtype(idx: int) -> Optional[torch.dtype]:
    ...
    if idx not in INT_TO_DTYPE:
        raise CorruptEncodedKVError(
            f"unknown dtype index {idx} in encoded header"
        )
...
def serialize_header(enc: EncodedKV) -> bytes:
    ...
    scale_shape_bytes = struct.pack(
        f"<{len(enc.scale_shape)}q", *enc.scale_shape
    )
...
def deserialize_header(buf: bytes) -> EncodedKV:
    ...
    if version != int(CodecVersion.V1):
        raise CorruptEncodedKVError(
            f"unsupported codec version {version}; this build supports {int(CodecVersion.V1)}"
        )
    ...
    scale_shape = tuple(
        struct.unpack_from(f"<{scale_shape_n}q", mv, off)
    )
    off += 8 * scale_shape_n
    ...
    payload = bytes(mv[off : off + expected_payload])

...
Quant convention (matches vLLM's asymmetric branch):
...
class AsymK16V8Codec:
    ...
    def __init__(
        ...
        *,
        ...
        if fp8_dtype == torch.float8_e5m2:
            raise UnsupportedConfigError(
                "float8_e5m2 is reserved-but-not-implemented in v1; "
                "use float8_e4m3fn"
            )
    ...
    def encode(
        ...
        *,
        ...
        If `precomputed_v_quant` is provided it is used as-is (this
        is the native_asym passthrough path: vLLM/FlashInfer has
        already produced V in FP8, we copy bytes without re-quanting).
        Otherwise V is quantized here from FP16/BF16.
        ...
        if precomputed_v_quant is not None:
            if precomputed_v_scales is None:
                raise UnsupportedConfigError(
                    "encode: precomputed_v_quant requires "
                    "precomputed_v_scales"
                )
        ...
        for key in CodecHashes._CHECK_ORDER:
            ...
            if ev and gv and ev != gv:
                raise CodecMismatchError(
                    f"codec hash mismatch on {key!r}: encoded={gv!r}, "
                    f"expected={ev!r}"
                )
    ...
    def decode(
        ...
        *,
        ...
        # K: simple frombuffer + view
        k_tensor = torch.frombuffer(
            bytearray(k_bytes), dtype=enc.k_dtype
        ).clone()
        ...
        scale_tensor = torch.frombuffer(
            bytearray(s_bytes), dtype=enc.scale_dtype
        ).clone().reshape(enc.scale_shape)
        scale_tensor = scale_tensor.to(device)

        # V: read as fp8, optionally dequantize
        v_q = torch.frombuffer(
            bytearray(v_bytes), dtype=enc.v_dtype
        ).clone().to(device)

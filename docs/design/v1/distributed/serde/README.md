...
The single-tensor `Serializer` / `Deserializer` interfaces above
cover symmetric per-tensor quantizers (one KV-shaped tensor in, one
KV-shaped tensor out). Some quantization schemes need a fixed-length
*tuple* of tensors at one of the endpoints. The motivating case is
asymmetric K16/V8: the deserialized form is a triple
`(K_native, V_fp8, V_scales)` so an asymmetric paged KV cache can
consume V without re-expanding it back to native dtype. V-only
writes are the dual: when K is already resident in HBM, only V (and
its scales) ship to L2, expressed as a tuple with the K slot left
absent.
...
- **Serialize input.** A `None` slot means the caller is not
  supplying that tensor; e.g., V-only writes when K is already
  resident in HBM and only V plus scales need to ship to L2. The
  implementation MUST raise `ValueError` when a required slot is
  `None`.
- **Deserialize output.** A `None` slot means the caller does not
  want that tensor materialized; e.g., V-only retrieval when K is
  already cached locally. The implementation MUST NOT touch the
  missing slot.

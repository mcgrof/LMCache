...
def test_kchild_slab_in_flight_tracks_one_shots() -> None:
    ...
    pool.  The PR-9 pod measurement showed that without this, the LRU
    policy was blind to most of the K-child footprint under heavy load
    (only 256 of 8000 K-children were in the pool; the other 7700 were
    invisible).
    """
    from lmcache.v1.distributed.l2_adapters.serde_wrapper import _KChildSlab
...
# =============================================================================
# PR-11': manifest-aware is_key_evictable — STORE_IN_FLIGHT K-children pinned
# =============================================================================
...
def test_kchild_pinned_during_store_in_flight() -> None:
    ...
    codec / L2 write (the PR-10 same-pod sweep reproducer)."""
...
def test_kchild_evictable_when_complete() -> None:
    ...
    paired eviction kicks in afterwards via PR-3'."""
    l1, manifest, k_child, logical = _build_l1_with_manifest()
...
def test_split_tier_inner_store_failure_deletes_v_children() -> None:
    ...
    delete because nothing was submitted (Finding C)."""
    # First Party
    from lmcache.v1.distributed.l2_adapters.serde_wrapper import (
        _StorePhase,
        _StoreTaskState,
...
def test_split_tier_rollback_release_never_drains_k_children() -> None:
    """Finding A: the failure-path K-child release must NOT fire the

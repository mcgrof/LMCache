...
def test_group_size_property_is_fixed() -> None:
    s = ConcatMultiSerializer(group_size=3)
    d = ConcatMultiDeserializer(group_size=3)
    assert s.group_size == 3
    assert d.group_size == 3
...
def test_validate_group_size_rejects_mismatch() -> None:
    with pytest.raises(ValueError, match="src"):
        validate_group_size((None, None), expected=3, role="src")
    with pytest.raises(ValueError, match="dst"):
        validate_group_size((None,), expected=2, role="dst")
...
def test_three_slot_roundtrip_all_present() -> None:
    s = ConcatMultiSerializer(group_size=3)
    d = ConcatMultiDeserializer(group_size=3)

    k = _bf16_tensor_obj(2, 4, 8, seed=1)
    v = _bf16_tensor_obj(2, 4, 8, seed=2)
    sc = _FakeMemoryObj(tensor=torch.tensor([0.5, 1.5, 2.5], dtype=torch.float32))
    src: MemoryObjGroup = (k, v, sc)

    layout = (
        MemoryLayoutDesc(shapes=[k.tensor.shape], dtypes=[k.tensor.dtype]),
        MemoryLayoutDesc(shapes=[v.tensor.shape], dtypes=[v.tensor.dtype]),
        MemoryLayoutDesc(shapes=[sc.tensor.shape], dtypes=[sc.tensor.dtype]),
    )
    ...
    sc_out = _FakeMemoryObj(tensor=torch.zeros_like(sc.tensor))
    d.deserialize(buf, (k_out, v_out, sc_out))

    assert torch.equal(k_out.tensor, k.tensor)
    assert torch.equal(v_out.tensor, v.tensor)
    assert torch.equal(sc_out.tensor, sc.tensor)


# =============================================================================
# None on serialize input: V-only writes (K slot absent)
# =============================================================================
...
def test_serialize_with_none_slot_skips_payload() -> None:
    ...
    sc = _FakeMemoryObj(tensor=torch.tensor([0.25, 0.75], dtype=torch.float32))
    src: MemoryObjGroup = (None, v, sc)
    ...
    expected = (
        3 * 5
        + v.tensor.numel() * v.tensor.dtype.itemsize
        + sc.tensor.numel() * sc.tensor.dtype.itemsize
    )
    assert n == expected
    ...
    v_out = _FakeMemoryObj(tensor=torch.zeros_like(v.tensor))
    sc_out = _FakeMemoryObj(tensor=torch.zeros_like(sc.tensor))
    d.deserialize(buf, (None, v_out, sc_out))
...
# =============================================================================
# None on deserialize output: V-only reads (skip K materialization)
# =============================================================================
...
def test_deserialize_with_none_slot_leaves_caller_buffer_untouched() -> None:
    ...
    sc = _FakeMemoryObj(tensor=torch.tensor([1.0], dtype=torch.float32))
    layout = (
        MemoryLayoutDesc(shapes=[k.tensor.shape], dtypes=[k.tensor.dtype]),
        MemoryLayoutDesc(shapes=[v.tensor.shape], dtypes=[v.tensor.dtype]),
        MemoryLayoutDesc(shapes=[sc.tensor.shape], dtypes=[sc.tensor.dtype]),

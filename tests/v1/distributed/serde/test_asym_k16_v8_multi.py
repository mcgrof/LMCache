...
serialize, ``(K_out, V_out)`` group on deserialize, K bit-exact and
V within FP8 noise. Native_asym mode and V-only writes are upstream
...
def test_serialize_rejects_none_slot_in_storage_only_mode() -> None:
    """Storage-only-dequant mode requires both K and V; None slots
    are reserved for the native-asym / V-only follow-up.
    """
    s = AsymK16V8MultiSerializer()
    buf = _byte_buffer(64)

...
def test_multi_output_local_roundtrip_preserves_named_outputs() -> None:
    ...
    n = s.serialize(_grp(*src), buf, _TEST_KEY)
    assert n > 0

    out = tuple(_byte_buffer(nbytes) for _, nbytes, _ in (_K, _V))
    d.deserialize(buf, _grp(*out), _TEST_KEY)
...
def test_single_output_bridge_backcompat() -> None:
    ...
    ms.serialize(_grp(payload), cast(MemoryObj, buf), _TEST_KEY)
    out = (_byte_buffer(1024),)
    md.deserialize(cast(MemoryObj, buf), _grp(*out), _TEST_KEY)
...
def test_multi_output_through_async_processor_roundtrip() -> None:
    ...
    try:
        ...
        sid = proc.submit_serialize(
            [cast(MemoryObj, src)], [cast(MemoryObj, buf)], [_TEST_KEY]
        )
        ...
        out = tuple(_byte_buffer(nbytes) for _, nbytes, _ in (_K, _V))
        did = proc.submit_deserialize(
            [cast(MemoryObj, buf)], [cast(MemoryObj, out)], [_TEST_KEY]
        )

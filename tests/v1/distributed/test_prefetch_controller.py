...
class TestPrefetchDrainSilentEgress:
    """B7/B31 + B8: prefetch must NOT fire the StoreController L1->L2 drain
    ...
    component / split-tier serde -- B7) or stores uninitialized buffers as
    garbage (B8)."""

    def test_warm_load_does_not_fire_store_drain(self, l1_manager):
        """B7/B31: a WARM prefetch makes loaded keys resident via the
    ...
    def test_trimmed_reservations_do_not_fire_store_drain(self, l1_manager):
        """B8: keys reserved during lookup but trimmed away (nothing

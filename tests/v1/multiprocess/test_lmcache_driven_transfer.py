...
"""Regression tests for the LMCache-driven object-group KV transfer.
...
``get_tensor(1)`` = V).  This asks whether that whole-object span is still
...
-------------------------------
When the native ``cuda_ops`` extension is absent (``NO_GPU_EXT=1`` build),
``lmcache.cuda_ops`` is transparently wired to the pure-Python fallback, so
...
* ``test_two_group_layout_is_packed_reinterpretation`` and
  ...
  drive the whole-object span through ``lmcache_memcpy_async_h2d`` /
  ``lmcache_memcpy_async_d2h``.  For a non-lazy ``MemoryObj`` these use a
  plain ``torch`` copy (no ``cuda_ops``), so they run on a ``NO_GPU_EXT`` pod
  and are the primary coverage there.
...
from lmcache.v1.memory_allocators.tensor_memory_allocator import TensorMemoryAllocator  # noqa: E402
...
import lmcache.lmcache_native as lmc_ops  # noqa: E402
...
class TestTwoGroupKvComponentTransfer:
    """Whole-object span correctness for 2-group KV_COMPONENT_GROUPS objects."""
    ...
    def test_two_group_object_staging_span_roundtrip(self) -> None:
        ...
        Drives the whole-object span primitive
        ...
        staging primitive uses a plain ``torch`` copy (no ``cuda_ops``).
        """
        packed_layout = _packed_layout()
        split_layout = apply_kv_component_split(packed_layout)
        ...
        # 2-group and packed produce byte-identical staged bytes.
    ...
    @pytest.mark.skipif(
        ...
        reason=(
            "native cuda_ops object-group transfer not compiled; the fallback "
